...
- [Tests and mocking the API](#tests-and-mocking-the-api)
  ...
  - [Unit tests with Jest](#unit-tests-with-jest)
    ...
    - [Unit test setup](#unit-test-setup)
      - [Unit test with some global Settings](#unit-test-with-some-global-settings)
...
Configuration for Jest is in `jest.config.js`, support files are in `tests/unit`, but as for the tests themselves - they're first-class citizens. That means they live alongside our source files, using the same name as the file they test, but with the extension `.spec.js`.
...
_Some configurations objects are_:

- testMatch

  Only tests `name.spec` files with `js`,`ts` extensions
  Tests all files with `js`,`ts` extensions under `__tests__` subfolder of any folder

  <pre>
    1.  **/tests/unit/**/_.spec.(js|jsx|ts|tsx)
    2.  **/__tests__/*.(js|jsx|ts|tsx)
  </pre>
...
- transform
  ...
  Your need to install all `vue-jest`, `ts-jest`, `babel-jest`
...
#### Unit test with some global Settings

- Include `vuetify`
- Include `shallowMount`
- Include `mount`
- Include component helpers

```javascript
// Globally using mixins
Vue.mixin({
  created() {
    this['$style'] = this['$style'] || {}
  }
})
// Globally using mountView, with slots for inject components
...
// Globally create a components with vue-router and vuex
