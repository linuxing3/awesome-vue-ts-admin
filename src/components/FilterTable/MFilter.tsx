...
import { WrappedFormUtils, FieldDecoratorOptions } from 'ant-design-vue/types/form/form';
...
class MFilterClass extends Vue {
  ...
  onSearch(): void {
    this.$emit(
      'search',
      Object.assign(this.params, (this.Form as WrappedFormUtils).getFieldsValue()),
    );
  }
  ...
  formItem(
    ...
    switch (item.type) {
      case 'input':
        itemDom = <a-input id={item.key} placeholder={item.placeholder} />;
        break;
      case 'select':
        itemDom = (
      ...
      case 'datetime':
        itemDom = (
          <a-date-picker id={item.key} showTime format="HH:mm:ss" placeholder={item.placeholder} />
        );
        break;
      case 'date':
        itemDom = (
          <a-date-picker id={item.key} format="YYYY-MM-DD" placeholder={item.placeholder} />
        );
        break;
      case 'datetimerange':
        itemDom = (
      ...
      case 'checkboxButton':
        itemDom = (
          <a-radio-group on-change={item.change} size="small">
            {item.options
  ...
  renderButtons(isNormal: boolean): JSX.Element {
    return (
      <div>
        <a-button on-click={this.onSearch} id="tableSearch" icon="search">
          Search
        </a-button>
        <a-button type="danger" on-click={this.reset} id="tableReset" icon="reload">
          Refresh
        </a-button>
        {this.filterGrade.length ? (
          <a on-click={() => this.gradeSwitch(isNormal)} class="grade-btn">
            {isNormal ? 'Common' : 'Senior'} Search
            {isNormal ? <i class="iconfont-down" /> : <i class="iconfont-up" />}
          </a>
        ) : null}
        <div class="right-btn">
          {this.addBtn ? (
            <a-button
              ...
              id={isNormal ? 'tableAdd' : 'tableAdd2'}
              icon="plus"
            >
              Add
            </a-button>
          ...
          {this.exportBtn ? (
            <a-button
              on-click={this.exportFun}
              id={isNormal ? 'tableExport' : 'tableExport2'}
              icon="download"
              shape="circle"
            />
          ) : null}
          <a-button on-click={this.openSetting} id="tableSet" icon="setting" shape="circle" />
        </div>
      </div>
    );
  }
